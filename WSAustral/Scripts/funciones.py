...
def Parser(txt, ListaGraficas, planta):
    ...
    for idx in range(len(txt)):
        ...
        if palabra == 'Embarcación':
            ...
            idx += 10
            # elif idx+2 < len(txt) and txt[idx] + txt[idx+1] + txt[idx+2] == "FechadeRegistro":
            #     sFechaRegistro = txt[idx+4]+" "+txt[idx+5]
            #     FechaRegistro = datetime.strptime(sFechaRegistro, '%d/%m/%Y %H:%M')
            ...
            if idx + 2 < len(txt) and txt[idx] + txt[idx + 1] + txt[idx + 2] == "DETALLEDECALA":
                ...
                # Me parece que esta tabla es FACT_FAENA_TERCEROS_CALAS *********************************
...
def borrar_calas(url):
    ...
    # url = "P:\Extraccion Calas - Jorge Castillo"
